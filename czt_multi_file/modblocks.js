let modblocks = [
   ...
   {normal: 12, reversed: 13, state: "normal", selected: false},{normal: 14, reversed: 15, state: "normal", selected: false}
]
function rerender_mods(){
   ...
   modblocks.forEach((modblock,index) => {
      ...
      let reverser = tag(30).html("&#x21BB;").addClass("reverser").on("click",(e) => {
         ...
         block.css("font-size", Math.min(1.2, 10 / Math.max(...modtext.text().split(" ").map(word => word.length)))+"em")
      })
      if(modblock.state == "normal"){
         modtext.html(mods[modblock.normal].name.toUpperCase());
      ...
      block.append(reverser,modtext).css("font-size", Math.min(1.2, 10 / Math.max(...modtext.text().split(" ").map(word => word.length)))+"em").on("click", () => {
